...
impl Editor {
    ...
    /// `mode` is >= 4 to represent forward, and mode % 4 == 1 for replace
    /// `forward` indicates whether to search forward or backward. If
    /// `do_rep` is true it will replace the last_match if it exists
    /// with `rep`. Returns the row of a new match, or `None` if the
    /// search was unsuccessful.
    #[allow(clippy::trivially_copy_pass_by_ref)] // This Clippy recommendation is only relevant on 32 bit platforms.
    fn find(
        &mut self, query: &str, rep: &str, last_match: Option<usize>, mode: u8,
    ) -> Option<usize> {
        let mut current = last_match.unwrap_or_else(|| self.rows.len().saturating_sub(1));
        let (num_rows, mut do_rep) = (self.rows.len(), mode % 4 == 1 && last_match.is_some());
        // TODO: Handle multiple matches per line
        for _ in 0..num_rows {
            // if do_rep we need to start on the last match to replace it
            current += if do_rep { 0 } else { if mode >= 4 { 1 } else { num_rows - 1 } };
            let row = &mut self.rows[current % num_rows];
            if let Some(cx) = slice_find(&row.chars, query.as_bytes()) {
                ...
                (self.cursor.x, self.cursor.y, self.cursor.coff) = (cx, current % num_rows, 0);
                row.match_segment = Some(row.cx2rx[cx]..row.cx2rx[cx] + query.len());
                if do_rep {
                    row.chars.splice(row.cx2rx[cx]..row.cx2rx[cx] + query.len(), rep.bytes());
                    (do_rep, row.match_segment) = (false, None);
                    self.update_row(current % num_rows, false);
                } else {
                    return Some(current % num_rows);
                }
            }
        }
        None
...
enum PromptMode {
    /// Save(prompt buffer)
    Save(String),
    /// Find(find prompt buffer, replace prompt buffer, saved cursor state, last
    /// match), when not in replace mode replace buffer should be '\n'
...
impl PromptMode {
    ...
    fn status_msg(&self) -> String {
        match self {
            ...
            Self::Find(fbuffer, rbuffer, ..) =>
                format!("Replace `{fbuffer}` (Use ESC/Arrows/Enter, ^R to replace): {rbuffer}"),
    ...
    fn process_keypress(self, ed: &mut Editor, key: &Key) -> Result<Option<Self>, Error> {
        ...
        match self {
            ...
            // find, replace, saved cursor, last match
            Self::Find(f, r, saved_cursor, last) => {
                if let Some(row_idx) = last {
                    ed.rows[row_idx].match_segment = None;
                }
                let m: u8 = if r == "\n" { 2 } else { 0 }; // m:mode, 2 = not currently in replace mode
                let (b, o) = if no_repl == 2 { (f, r) } else { (r, f) }; // choose which is the active buffer and which is the other buffer
                match process_prompt_keypress(b, key) {
                    PromptState::Active(prompted) => {
                        #[allow(clippy::wildcard_enum_match_arm)]
                        let (last, m) = match key {
                            // last match, mode has 4 for forward, and 1 for replace action
                            Key::Char(REPLACE) => (last, 5 + m),
                            Key::Arrow(AKey::Right | AKey::Down) | Key::Char(FIND) => (last, 4 + m),
                            Key::Arrow(AKey::Left | AKey::Up) => (last, m),
                            _ => (None, 4 + m),
                        };
                        // get them back based on which is the active buffer, if we are just
                        // entering replace (not in replace mode, and replace action) we clear the
                        // replace buffer
                        let n = if m % 4 >= 2 { String::new() } else { o }; // not yet replace mode
                        let (f, r) = if m % 2 == 1 { (prompted, n) } else { (o, prompted) }; // replace action
                        let (fnd, mut curr) = (m % 4 != 3 || !last.is_some(), last); // replace && not yet replace mode
                        // we use the last match if we are entering replace for the first time so
                        // that we don't go to a new entry
                        curr = if fnd { ed.find(&f, &r, last, m) } else { curr };
                        return Ok(Some(Self::Find(f, r, saved_cursor, curr)));
                    }
